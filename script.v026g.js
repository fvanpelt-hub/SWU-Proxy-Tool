...
(function(){
  ...
  // Draw 2x4 grid trim guides (baked-in, asset-free)
  function drawTrimGuides(ctx, geom, cols, rows, startX, startY, CW, CH){
    ctx.save();
    // stroke ~3px at 300dpi
    var px = Math.max(2, Math.round(geom.DPI/150));
    ctx.lineWidth = px;
  ...
  function render(){
    ...
    return loadOverlay().then(function(){
      ...
      // baked-in trim guides (always on)
      chain = chain.then(function(){
        drawTrimGuides(ctx, geom, cols, rows, startX, startY, CW, CH);
      });
var sb = $('#statusBar');
