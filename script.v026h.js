...
(function(){
  ...
  var state = { names: [], overlayBmp: null,
      overlayUnderBmp: null, overlayPath: null };
  ...
  function render(){
    ...
    ctx.fillStyle = '#fff'; ctx.fillRect(0,0,geom.PAGE_W, geom.PAGE_H);
      // draw underlay (below cards)
      chain = chain.then(function(){
        if (state.overlayUnderBmp){
          ctx.save(); ctx.globalAlpha = 1;
          ctx.drawImage(state.overlayUnderBmp, 0, 0, geom.PAGE_W, geom.PAGE_H);
          ctx.restore();
        }
      });
    
    return loadOverlay().then(function(){
      /* overlay will be drawn after cards */
      if (geom.BL > 0){
        ctx.fillStyle = '#000';
      ...
      // draw overlay on TOP after all cards (ALWAYS ON, opacity 1)
      chain = chain.then(function(){
        if (state.overlayBmp){
          ctx.save(); ctx.globalAlpha = 1;
...
var sb = $('#statusBar');
  ...
  // Preload default overlay assets (underlay PNG + overlay SVG)
  async function preloadDefaults(){
    try {
      const r1 = await fetch('assets/template_resized_1056x816.png');
      if (r1.ok) {
        const bmp1 = await createImageBitmap(await r1.blob());
        state.overlayUnderBmp = bmp1;
      }
    } catch(e){ console.warn('[swu-sheet] default underlay missing', e); }
    try {
      const r2 = await fetch('assets/letter_poker_v2_fixed.svg');
      if (r2.ok) {
        const bmp2 = await createImageBitmap(await r2.blob());
        state.overlayBmp = bmp2;
      }
    } catch(e){ console.warn('[swu-sheet] default overlay missing', e); }
  }
  ...
  else init(); preloadDefaults().then(render);
})();