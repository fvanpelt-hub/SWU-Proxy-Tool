...
(function(){
  ...
  function render(){
    ...
    return loadOverlay().then(function(){
      ...
      // draw overlay on TOP after all cards
      chain = chain.then(function(){
        var _on = ($('#overlayToggle') && $('#overlayToggle').checked) || false;
        var _op = parseFloat(($('#overlayOpacity') && $('#overlayOpacity').value) || '1');
        if (_on && state.overlayBmp){
          ctx.save(); ctx.globalAlpha = clamp(_op, 0, 1);
