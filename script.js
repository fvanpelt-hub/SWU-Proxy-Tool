...
// ---- State ----
const state = {
  dpi: 300, cardW_in: 2.5, cardH_in: 3.5, bleed_mm: 0.5,
  cols: 4, rows: 2, marginL_in: 0.5, marginT_in: 0.75,
  pageW_in: 11, pageH_in: 8.5,
  sheets: [ { slots: [] } ], sheetIndex: 0,
  catalog: [], overlayOpacity: 0.2, overlayVisible: true,
};
...
const buttons = {
  prev: document.getElementById('btnPrev'), next: document.getElementById('btnNext'),
  png: document.getElementById('btnSavePNG'), print: document.getElementById('btnPrint'),
  parse: document.getElementById('btnParse'), addSearch: document.getElementById('btnAddSearch'),
  imgUpload: document.getElementById('imgUpload'), clearSheet: document.getElementById('btnClearSheet'),
  clearAll: document.getElementById('btnClearAll'), dxf: document.getElementById('btnExportDXF'),
  saveJSON: document.getElementById('btnSaveJSON'), loadJSON: document.getElementById('btnLoadJSON'),
};
...
function saveLocal(){
  const toSave = { ...state, sheets: state.sheets.map(s => ({
...
function render(){
  ...
  const cardW_px=px(state.cardW_in), cardH_px=px(state.cardH_in);
  const bleed_px=px(mm2in(state.bleed_mm));
  const marginL_px=px(state.marginL_in), marginT_px=px(state.marginT_in);
  const ctx=sheetCanvas.getContext('2d'); ctx.fillStyle='#000'; ctx.fillRect(0,0,pageW_px,pageH_px);
  ctx.strokeStyle='#fff'; ctx.lineWidth=3; ctx.strokeRect(30, pageH_px-30-px(1), px(1), px(1));
  const sheet=currentSheet(); ensureSlots(sheet);
  for(let r=0;r<state.rows;r++){
    for(let c=0;c<state.cols;c++){
      const idx=r*state.cols+c, slot=sheet.slots[idx]||{};
      const x=marginL_px+c*cardW_px, y=marginT_px+r*cardH_px;
      ctx.fillStyle='#000'; ctx.fillRect(x-bleed_px,y-bleed_px,cardW_px+2*bleed_px,cardH_px+2*bleed_px);
      if(slot.img){ ctx.drawImage(slot.img,x,y,cardW_px,cardH_px); }
      else{ ctx.fillStyle='#141922'; ctx.fillRect(x,y,cardW_px,cardH_px); ctx.strokeStyle='#273142'; ctx.lineWidth=2; ctx.strokeRect(x,y,cardW_px,cardH_px); ctx.fillStyle='#9aa3ad'; ctx.font='16px sans-serif'; ctx.fillText('Empty', x+10, y+24); }
    }
  }
  renderThumb(); saveLocal(); updatePageLabel();
}
function renderThumb(){
  const sheet=currentSheet(), thumb=document.createElement('canvas'); const scale=300/3300;
  thumb.width=Math.round(sheetCanvas.width*scale); thumb.height=Math.round(sheetCanvas.height*scale);
  const tctx=thumb.getContext('2d'); tctx.drawImage(sheetCanvas,0,0,thumb.width,thumb.height);
  sheet.thumbDataUrl=thumb.toDataURL('image/png'); thumbStrip.innerHTML='';
  state.sheets.forEach((s,i)=>{ const img=document.createElement('img'); img.src=s.thumbDataUrl||''; img.alt=`Sheet ${i+1}`; img.onclick=()=>{ state.sheetIndex=i; render(); }; thumbStrip.appendChild(img); });
}
...
// ---- DXF export ----
function exportDXF(){
  const pageW_in=state.pageW_in, pageH_in=state.pageH_in, cardW=state.cardW_in, cardH=state.cardH_in;
  const cols=state.cols, rows=state.rows, marginL=state.marginL_in, marginT=state.marginT_in;
  let entities=''; for(let r=0;r<rows;r++){ for(let c=0;c<cols;c++){ const x=marginL+c*cardW, y=marginT+r*cardH; const pts=[[x,y],[x+cardW,y],[x+cardW,y+cardH],[x,y+cardH],[x,y]]; entities+=polyline(pts); } }
  const dxf=headerDXF()+entities+footerDXF(); const blob=new Blob([dxf],{type:'application/dxf'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`swu_proxy_cutlines_${Date.now()}.dxf`; a.click();
}
...
// ---- Events ----
buttons.png.onclick=()=>{ sheetCanvas.toBlob(blob=>{ const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`swu_sheet_${state.sheetIndex+1}_${state.dpi}dpi.png`; a.click(); }); };
...
buttons.parse.onclick=async ()=>{
  const text=controls.pasteList.value.trim(); if(!text) return;
  const items=parseLines(text); for(const it of items){ await addCardByName(it.name, it.qty); }
};
buttons.addSearch.onclick=async ()=>{ const name=controls.searchBox.value.trim(); if(name) await addCardByName(name,1); };
buttons.imgUpload.onchange=async (e)=>{ const files=Array.from(e.target.files||[]); for(const f of files){ const url=URL.createObjectURL(f); await addImageByURL(url,f.name); } };

controls.searchBox.addEventListener('input', ()=>{
  const q=normalizeName(controls.searchBox.value.trim()); if(!q){ controls.autocomplete.style.display='none'; return; }
  const matches=Array.isArray(state.catalog) ? state.catalog.filter(n=>normalizeName(n).includes(q)) : [];
  const box=controls.autocomplete; box.innerHTML='';
  matches.slice(0,12).forEach(n=>{ const div=document.createElement('div'); div.textContent=n; div.onclick=()=>{ controls.searchBox.value=n; box.style.display='none'; }; box.appendChild(div); });
  box.style.display = matches.length ? 'block' : 'none';
});
document.addEventListener('click', (e)=>{ if(!controls.autocomplete.contains(e.target) && e.target!==controls.searchBox){ controls.autocomplete.style.display='none'; } });
